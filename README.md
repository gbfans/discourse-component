...
2. Set the palette's `quaternary` slot to `#050505` in the dark palette and `#353535` in the light/default palette — this drives `--gbfans-page-bg` (the tiled backdrop behind the panel), which has no dedicated Discourse palette role. (`quaternary` is confirmed to exist as a real root CSS custom property; not every admin-editable color field does — see the note on `--gbfans-nav-bg` below.)
...
`--gbfans-nav-bg` (the desktop nav bar) needs no admin configuration — it's computed as `color-mix(in srgb, var(--secondary) 92%, black 8%)`. Next.js's nav bar is always a shade darker than the surface behind it in both themes, which Discourse's auto-derived `-low`/`-high` shades can't express (they only ever interpolate between `primary` and `secondary`, so they can't go darker than `secondary` itself in dark mode). Earlier attempts assumed the color picker's "selected" and "hover" fields were also exposed as `--selected` / `--hover` CSS custom properties like `quaternary` is — they are not, at least not on Discourse's current color-scheme implementation; they read back empty. If you ever see a `--gbfans-*` token rendering as `unset`/inherited instead of a color, check the browser console for whether the palette var it depends on actually resolves (`getComputedStyle(document.documentElement).getPropertyValue('--the-var')`) before assuming it's a config problem.
